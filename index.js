...
var ak = "G8C5dBGDOP8ldUVo"
var sk = "Hm5a5LsNqCllOXFjvfWUupT77mYbFKjJ"
var token = "NETLESSSDK_YWs9RzhDNWRCR0RPUDhsZFVWbyZub25jZT0yNWU3MjE1MC04MWVkLTExZWQtODYzMS1hNzA1MzkzY2M0YTkmcm9sZT0wJnNpZz1jODIzODQ4NDUyY2M4ZTU1Nzc4NWVhNmZkMTI1ODZmMjk4ZjBjZDJhYWZlMjFlMGM2YTZkM2NiMzlmODFiNzE2"
...
io.on("connection", (socket) => {
  ...
  socket.on("/enterRoom",(data) => {
    if(players.length == 8)
    {
      ...
      players.push(player),
      console.log("Room joined: " + data["roomId"] + " by " +data["playerName"])
      // socket.to("abc").emit("success","true")
      console.log(data)
      socket.emit("roomJoined",data["socketId"]+":"+data["roomId"]+":"+data["playerName"])
      socket.to(data["roomId"]).emit("roomJoined",data["socketId"]+":"+data["roomId"]+":"+data["playerName"])
  ...
  socket.on("getPlayers" , (data) => {
    ...
    {
      data += players[x].playerName + ":" + players[x].roomId + ":" + players[x].socketId + ":";
    }
    console.log(data);
  ...
  socket.on("leaveRoom", (room) => {
    ...
    while(i<players.length)
    {
      ...
      {
      data += players[i].playerName + ":" + players[i].roomId + ":" + players[i].socketId + ":";
      }
      i++;
    }
  ...
  socket.on("getSdkToken",async (data) => {
    ...
    const  { sdkToken, TokenPrefix1 } = require("./node_modules/netless-token/dist/index.js" );
  ...
  socket.on("getRoomToken",(roomUUID) => {
    ...
    const  { roomToken, TokenPrefix2 } = require("./node_modules/netless-token/dist/index.js" );
  ...
  socket.on("startGame", (data) => {
    console.log("startGame event triggered.. turn=" + currentTurn + "total players: " +players.length)
    gameStarted = true;
    socket.to(data).emit("startGameResponse",players[currentTurn].playerName + ":" + players[currentTurn].socketId);
    socket.emit("startGameResponse",players[currentTurn].playerName+ ":" + players[currentTurn++].socketId)
  ...
  socket.on("nextTurn" , (data) => {
    ...
    socket.to(data).emit("nextTurnResponse",players[currentTurn].playerName+ ":" + players[currentTurn].socketId);
    socket.emit("nextTurnResponse",players[currentTurn].playerName+ ":" + players[currentTurn++].socketId)
    if(currentTurn == players.length)
      currentTurn=0;
  })
  ...
  socket.on("sendMessage",(data) => {
    ...
    {
      socket.to(data["roomId"]).emit("receiveMessage",senderName+":" +senderName + " guessed correct word")
  ...
  socket.on("test2",(data) => {
    console.log(data),
    socket.to("Room1").emit("event","Someone joined...")
  })

  socket.on("test",(data) => {
    socket.join("abc")
    console.log(data),
    socket.emit("success","you joined")
